...
// Detailed installation tracking by location
// ═══════════════════════════════════════════════════════════════

App.renderInfrastructure = async function (c) {
    c.innerHTML = `<div class="page-header"><div><h1 class="page-title">🏢 Infrastructure</h1><p class="page-subtitle">MDF / IDF installation tracking — material & labor budget control</p></div><div class="page-actions"><button class="btn btn-primary" id="btn-add-loc">+ Add Location</button></div></div><div id="infra-totals" class="metric-grid" style="margin-bottom:20px;"></div><div id="infra-locations"><div class="empty-state"><div class="empty-state-icon">⏳</div><div class="empty-state-title">Loading...</div></div></div>`;
    ...
    if (locations.length === 0) {
        list.innerHTML = `<div class="empty-state"><div class="empty-state-icon">🏢</div><div class="empty-state-title">No Locations Yet</div><div class="empty-state-desc">Add MDF and IDF rooms to start tracking installations.</div></div>`;
    } else {
    ...
    document.getElementById('btn-add-loc').addEventListener('click', () => this.addLocation());
};
...
App.viewLocation = async function (locId) {
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">${[loc.building, loc.floor ? 'Floor ' + loc.floor : '', loc.room_number ? 'Rm ' + loc.room_number : ''].filter(Boolean).join(' · ') || 'Infrastructure Location'}</p>
      </div>
      <div class="page-actions">
        <button class="btn btn-secondary" id="btn-back-infra">← Back</button>
        <button class="btn btn-danger" id="btn-del-loc" style="font-size:12px;">🗑 Delete</button>
      </div>
    </div>
    ...
    <div id="tab-equipment" class="infra-tab-content">
      <div style="margin-bottom:12px;"><button class="btn btn-primary btn-sm" id="btn-add-item">+ Add Equipment</button></div>
      ${items.length === 0 ? '<div class="card" style="padding:30px;text-align:center;color:var(--text-muted);">No equipment added yet.</div>' :
            `<div class="card"><table class="data-table"><thead><tr><th>Category</th><th>Item</th><th>Model</th><th style="text-align:right">Budget Qty</th><th style="text-align:right">Installed</th><th style="text-align:right">Budget $</th><th style="text-align:right">Actual $</th><th>Status</th><th></th></tr></thead><tbody>${items.map(i => {
                const over = i.actual_cost > i.budgeted_cost && i.budgeted_cost > 0;
                return `<tr><td><span class="badge badge--sky" style="font-size:10px;">${esc(i.category)}</span></td><td><strong>${esc(i.item_name)}</strong></td><td style="font-size:12px;">${esc(i.model || '—')}</td><td style="text-align:right">${i.budgeted_qty} ${esc(i.unit)}</td><td style="text-align:right;font-weight:600;">${i.installed_qty} ${esc(i.unit)}</td><td style="text-align:right">$${formatMoney(i.budgeted_cost)}</td><td style="text-align:right;font-weight:600;${over ? 'color:var(--error)' : ''}">$${formatMoney(i.actual_cost)}${over ? ' ⚠' : ''}</td><td><span class="badge badge--${i.status === 'installed' || i.status === 'tested' ? 'active' : i.status === 'ordered' ? 'amber' : 'draft'}">${formatStatus(i.status)}</span></td><td><button class="btn-icon" onclick="App.editItem('${i.id}',${JSON.stringify(i).replace(/"/g, '&quot;')})">✏️</button><button class="btn-icon" onclick="App.deleteInfraEntity('item','${i.id}')">🗑️</button></td></tr>`;
            }).join('')}</tbody></table></div>`}
    </div>

    <!-- CABLE RUNS TAB -->
    <div id="tab-runs" class="infra-tab-content" style="display:none;">
      <div style="margin-bottom:12px;"><button class="btn btn-primary btn-sm" id="btn-add-run">+ Add Cable Run</button></div>
      ${runs.length === 0 ? '<div class="card" style="padding:30px;text-align:center;color:var(--text-muted);">No cable runs added yet.</div>' :
            `<div class="card"><table class="data-table"><thead><tr><th>Label</th><th>Type</th><th>Destination</th><th>Pathway</th><th style="text-align:right">Budget ft</th><th style="text-align:right">Installed ft</th><th style="text-align:right">Labor Hrs</th><th>Status</th><th></th></tr></thead><tbody>${runs.map(r => {
                const labOver = r.actual_labor_hrs > r.budgeted_labor_hrs && r.budgeted_labor_hrs > 0;
                return `<tr><td><strong>${esc(r.run_label || '—')}</strong></td><td><span class="badge badge--indigo" style="font-size:10px;">${esc(r.cable_type)}</span></td><td>${esc(r.destination)}${r.destination_floor ? ' (Fl ' + esc(r.destination_floor) + ')' : ''}</td><td style="font-size:12px;">${esc(r.pathway || '—')}</td><td style="text-align:right">${formatMoney(r.budgeted_qty)}</td><td style="text-align:right;font-weight:600;">${formatMoney(r.installed_qty)}</td><td style="text-align:right;${labOver ? 'color:var(--error);font-weight:600' : ''}">${(r.actual_labor_hrs || 0).toFixed(1)} / ${(r.budgeted_labor_hrs || 0).toFixed(1)}${labOver ? ' ⚠' : ''}</td><td><span class="badge badge--${r.status === 'tested' || r.status === 'labeled' ? 'active' : r.status === 'pulled' || r.status === 'terminated' ? 'amber' : 'draft'}">${formatStatus(r.status)}</span></td><td><button class="btn-icon" onclick="App.editRun('${r.id}',${JSON.stringify(r).replace(/"/g, '&quot;')})">✏️</button><button class="btn-icon" onclick="App.deleteInfraEntity('run','${r.id}')">🗑️</button></td></tr>`;
            }).join('')}</tbody></table></div>`}
    </div>

    <!-- LABOR TAB -->
    <div id="tab-labor" class="infra-tab-content" style="display:none;">
      <div style="margin-bottom:12px;"><button class="btn btn-primary btn-sm" id="btn-add-labor">+ Add Labor Entry</button></div>
      ${labor.length === 0 ? '<div class="card" style="padding:30px;text-align:center;color:var(--text-muted);">No labor entries added yet.</div>' :
            `<div class="card"><table class="data-table"><thead><tr><th>Task</th><th>Description</th><th>Date</th><th style="text-align:right">Crew</th><th style="text-align:right">Budget Hrs</th><th style="text-align:right">Actual Hrs</th><th></th></tr></thead><tbody>${labor.map(l => {
                const over = l.actual_hours > l.budgeted_hours && l.budgeted_hours > 0;
                return `<tr><td><span class="badge badge--amber" style="font-size:10px;">${formatStatus(l.task_type)}</span></td><td>${esc(l.description || '—')}</td><td>${formatDate(l.date_worked)}</td><td style="text-align:right">${l.worker_count}</td><td style="text-align:right">${(l.budgeted_hours || 0).toFixed(1)}</td><td style="text-align:right;font-weight:600;${over ? 'color:var(--error)' : ''}">${(l.actual_hours || 0).toFixed(1)}${over ? ' ⚠' : ''}</td><td><button class="btn-icon" onclick="App.editLabor('${l.id}',${JSON.stringify(l).replace(/"/g, '&quot;')})">✏️</button><button class="btn-icon" onclick="App.deleteInfraEntity('labor','${l.id}')">🗑️</button></td></tr>`;
            }).join('')}</tbody></table></div>`}
    </div>`;
    ...
    document.getElementById('btn-add-item').addEventListener('click', () => this.addEquipment(locId));
    document.getElementById('btn-add-run').addEventListener('click', () => this.addCableRun(locId));
    document.getElementById('btn-add-labor').addEventListener('click', () => this.addLaborEntry(locId));
};

// ── Add Equipment ──────────────────────────────────────────
App.addEquipment = function (locId) {
    const pid = this.state.projectId;
    const cats = ['rack', 'switch', 'patch_panel', 'fiber_panel', 'ups', 'pdu', 'cable_management', 'server', 'media_converter', 'junction_box', 'backboard', 'grounding', 'conduit', 'other'];
    this.showModal('Add Equipment / Material', `<div class="form-grid">
      ...
      <div class="form-group"><label class="form-label">Budgeted Qty</label><input class="form-input" id="ei-bqty" type="number" step="1" value="0"></div>
      ...
      <div class="form-group form-full"><label class="form-label">Notes</label><textarea class="form-input" id="ei-notes" rows="2"></textarea></div>
        ...
        const bqty = parseFloat(document.getElementById('ei-bqty').value) || 0;
        const ucost = parseFloat(document.getElementById('ei-ucost').value) || 0;
...
App.addCableRun = function (locId) {
    ...
    this.showModal('Add Cable Run', `<div class="form-grid">
      ...
      <div class="form-group"><label class="form-label">Budgeted Length (ft)</label><input class="form-input" id="cr-bqty" type="number" step="1" value="0"></div>
      ...
      <div class="form-group"><label class="form-label">Status</label><select class="form-select" id="cr-status"><option value="planned">Planned</option><option value="pulled">Pulled</option><option value="terminated">Terminated</option><option value="tested">Tested</option><option value="labeled">Labeled</option></select></div>
        ...
        const r = await API.locationAction(pid, locId, { action: 'add_run', run_label: document.getElementById('cr-label').value.trim(), cable_type: document.getElementById('cr-type').value, destination: dest, destination_floor: document.getElementById('cr-dfloor').value.trim(), pathway: document.getElementById('cr-path').value, budgeted_qty: parseFloat(document.getElementById('cr-bqty').value) || 0, budgeted_labor_hrs: parseFloat(document.getElementById('cr-blabor').value) || 0, status: document.getElementById('cr-status').value });
        if (r.error) { this.toast(r.error, 'error'); return; }
...
App.addLaborEntry = function (locId) {
    ...
    this.showModal('Add Labor Entry', `<div class="form-grid">
      ...
      <div class="form-group"><label class="form-label">Budgeted Hours</label><input class="form-input" id="lb-bhrs" type="number" step="0.5" value="0"></div>
      ...
      <div class="form-group form-full"><label class="form-label">Notes</label><textarea class="form-input" id="lb-notes" rows="2"></textarea></div>
        ...
        const r = await API.locationAction(pid, locId, { action: 'add_labor', task_type: document.getElementById('lb-type').value, description: document.getElementById('lb-desc').value.trim(), date_worked: document.getElementById('lb-date').value || null, worker_count: parseInt(document.getElementById('lb-crew').value) || 1, budgeted_hours: parseFloat(document.getElementById('lb-bhrs').value) || 0, actual_hours: parseFloat(document.getElementById('lb-ahrs').value) || 0, notes: document.getElementById('lb-notes').value.trim() });
        if (r.error) { this.toast(r.error, 'error'); return; }
        this.closeModal(); this.toast('Labor entry added', 'success'); this.viewLocation(locId);
    });
};

// ── Edit Equipment (update installed qty, actual cost, status) ──
...
App.editLabor = function (laborId, lab) {
    ...
    this.showModal('Update Labor Entry', `<div class="form-grid">
      ...
      <div class="form-group"><label class="form-label">Budgeted Hours</label><input class="form-input" id="ul-bhrs" type="number" step="0.5" value="${lab.budgeted_hours || 0}"></div>
      <div class="form-group"><label class="form-label">Crew Size</label><input class="form-input" id="ul-crew" type="number" step="1" value="${lab.worker_count || 1}"></div>
        ...
        const r = await API.locationAction(pid, locId, { action: 'update_labor', labor_id: laborId, actual_hours: parseFloat(document.getElementById('ul-ahrs').value) || 0, budgeted_hours: parseFloat(document.getElementById('ul-bhrs').value) || 0, worker_count: parseInt(document.getElementById('ul-crew').value) || 1 });
        if (r.error) { this.toast(r.error, 'error'); return; }
